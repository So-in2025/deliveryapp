...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className={`h-[100dvh] w-full flex flex-col items-center overflow-hidden ${bgClass}`}>
      ...
      <div className={`w-full max-w-md h-full shadow-2xl relative flex flex-col overflow-hidden ${containerBgClass}`}>
        ...
        {/* Header - ONLY show if NOT in Dev Mode, NOT Admin (Admin has custom header), AND NOT in Auth Mode */}
        ...
        <main className={`flex-1 overflow-y-auto scrollbar-hide relative ${(!isDev && !isAdmin) ? 'pb-safe' : ''}`}>
          {children}
        </main>

        {/* Bottom Navigation - Standard Roles */}
        {!isDev && !isAuth && !isAdmin && (
          <nav className="shrink-0 w-full bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 pb-safe pt-2 px-6 flex justify-between items-center z-40 relative transition-colors duration-300">
        ...
        {/* Bottom Navigation - Admin Only (Simplified) */}
        {isAdmin && (
            <nav className="shrink-0 w-full bg-slate-900 border-t border-slate-800 pb-safe pt-2 px-6 flex justify-between items-center z-40 relative text-slate-400">
             <button 
                onClick={() => setRole(UserRole.NONE)}
