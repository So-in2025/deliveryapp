...
export const IOSInstallPrompt: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col justify-end pointer-events-none">
      ...
      {/* Drawer */}
      <div className="bg-white/95 backdrop-blur-xl border-t border-stone-200 p-6 pb-safe rounded-t-3xl shadow-2xl pointer-events-auto animate-slide-up relative">
        <button 
            onClick={() => setShowPrompt(false)}
            className="absolute top-4 right-4 text-stone-400 hover:text-stone-600 bg-stone-100 p-1 rounded-full"
        >
            <X size={16} />
        </button>
        ...
        <div className="flex gap-4 items-start pr-8">
             ...
             <div>
                 <h3 className="font-bold text-stone-900 text-lg">Instalar App</h3>
        ...
        <div className="mt-6 space-y-3">
            <div className="flex items-center gap-3 text-sm text-stone-700">
                <span className="flex items-center justify-center w-8 h-8 bg-stone-100 rounded-lg text-amber-600">
                    <Share size={18} />
                </span>
                <span>1. Toca el botón <b>Compartir</b> abajo.</span>
            </div>
            <div className="flex items-center gap-3 text-sm text-stone-700">
                <span className="flex items-center justify-center w-8 h-8 bg-stone-100 rounded-lg text-stone-600">
                    <PlusSquare size={18} />
                </span>
        ...
        <div className="absolute -bottom-2 left-1/2 transform -transtone-x-1/2 w-4 h-4 bg-white rotate-45 border-b border-r border-stone-200"></div>
      </div>
    </div>
  );
};