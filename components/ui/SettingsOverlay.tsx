...
export const SettingsOverlay: React.FC = () => {
  ...
  const MainView = () => (
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-slate-50 dark:bg-slate-950 animate-slide-in-right transition-colors duration-300">
            {/* Partners Hub */}
            <div>
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Zona Partners</h3>
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-colors duration-300">
                    ...
                    <div 
                        ...
                        className="flex items-center justify-between p-4 border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer group transition-colors"
                    >
                    ...
                    <div 
                        ...
                        className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer group transition-colors"
                    >
                        <div className="flex items-center gap-3 text-slate-700 dark:text-slate-300">
            ...
            {/* General Settings */}
            <div>
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">General</h3>
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-colors duration-300">
             ...
             <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-colors duration-300">
                <div 
                    onClick={handleReset}
                    className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer text-red-600 dark:text-red-400 transition-colors"
                >
            ...
            <div className="bg-slate-900 rounded-2xl shadow-sm border border-slate-800 overflow-hidden p-4">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                    <Sparkles size={12} className="text-yellow-400" /> Demo: Cambiar Rol
                </h3>
                <div className="grid grid-cols-4 gap-2 mb-3">
                    <button onClick={() => switchToRole(UserRole.CLIENT)} className="p-2 bg-slate-800 rounded-lg text-xs font-bold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors">Cliente</button>
                    <button onClick={() => switchToRole(UserRole.MERCHANT, demoStoreId)} className="p-2 bg-slate-800 rounded-lg text-xs font-bold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors">Comercio</button>
                    <button onClick={() => switchToRole(UserRole.DRIVER)} className="p-2 bg-slate-800 rounded-lg text-xs font-bold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors">Driver</button>
                    <button onClick={() => switchToRole(UserRole.ADMIN)} className="p-2 bg-slate-800 rounded-lg text-xs font-bold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors">Admin</button>
                </div>
                {role === UserRole.MERCHANT && (
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] text-slate-500 uppercase font-bold">Tienda Demo:</span>
                        <select 
                            value={demoStoreId}
                            onChange={(e) => {
                            ...
                            className="flex-1 bg-slate-800 border border-slate-700 text-slate-300 text-xs rounded p-1 outline-none focus:border-brand-500"
                        >
                            {stores.map(s => (
            ...
            <div className="px-2">
                <p className="text-xs text-center text-slate-400 mb-4">Versión 1.0.8 (Onboarding)</p>
                <button 
  ...
  const EditProfileView = () => (
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-slate-50 dark:bg-slate-950 animate-slide-in-right transition-colors duration-300">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 text-center transition-colors duration-300">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Toca para cambiar</p>
          </div>

          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 space-y-4 transition-colors duration-300">
              <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Nombre Completo</label>
                  <input 
                    value={editName}
                    onChange={(e) => setEditName(e.target.value)}
                    className="w-full mt-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 outline-none focus:border-brand-500 font-medium text-slate-900 dark:text-white transition-colors"
                  />
              </div>
              <div>
  ...
  const RegisterMerchantView = () => (
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-slate-50 dark:bg-slate-950 animate-slide-in-right transition-colors duration-300">
          <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-purple-900/20">
              ...
              <p className="text-purple-100 text-sm mt-1">Empieza a vender en minutos. Completamente gratis para empezar.</p>
          </div>

          <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 space-y-5 transition-colors duration-300">
              <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Nombre del Local</label>
                  <input 
                    placeholder="Ej: Burger King"
                    value={storeName}
                    onChange={(e) => setStoreName(e.target.value)}
                    className="w-full mt-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 outline-none focus:border-purple-500 font-medium text-slate-900 dark:text-white transition-colors"
                  />
              </div>
              ...
              <div>
                  ...
                  <select 
                    ...
                    className="w-full mt-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 outline-none focus:border-purple-500 font-medium appearance-none text-slate-900 dark:text-white transition-colors"
                  >
              ...
              <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Tiempo de Entrega (min)</label>
                  <div className="flex items-center gap-2 mt-1">
                      ...
                      <input 
                        ...
                        className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 outline-none focus:border-purple-500 font-medium text-slate-900 dark:text-white transition-colors"
                      />
                  </div>
              </div>
          </div>

          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl flex gap-3 items-start text-sm text-blue-800 dark:text-blue-400 border border-blue-100 dark:border-blue-900/50 transition-colors duration-300">
               <Check size={18} className="shrink-0 mt-0.5" />
               <p>Al crear tu tienda, aparecerás inmediatamente en la sección <b>"Nuevos en la App"</b> con visibilidad destacada.</p>
          </div>

          <Button fullWidth onClick={handleRegisterMerchant} disabled={!storeName} className="bg-purple-600 hover:bg-purple-700 shadow-purple-500/30">
              Lanzar Negocio
          </Button>
      </div>
  );
  ...
  const RegisterDriverView = () => (
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-slate-50 dark:bg-slate-950 animate-slide-in-right transition-colors duration-300">
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 space-y-5 transition-colors duration-300">
              <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Tipo de Vehículo</label>
                  <div className="grid grid-cols-2 gap-3 mt-2">
                      {['Moto', 'Bici', 'Auto', 'Pie'].map(v => (
                          <div 
                            key={v}
                            onClick={() => setDriverVehicle(v)}
                            className={`p-3 rounded-xl border text-center font-bold text-sm cursor-pointer transition-all ${driverVehicle === v ? 'bg-amber-50 dark:bg-amber-900/30 border-amber-500 text-amber-700 dark:text-amber-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'}`}
                          >
                              {v}
                          </div>
                      ))}
              ...
              <div>
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Patente / ID (Opcional)</label>
                  <input 
                    placeholder="ABC-123"
                    value={driverPlate}
                    onChange={(e) => setDriverPlate(e.target.value)}
                    className="w-full mt-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 outline-none focus:border-amber-500 font-medium uppercase text-slate-900 dark:text-white transition-colors"
                  />
              </div>
          </div>

           <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl flex gap-3 items-center text-sm text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 transition-colors duration-300">
               <MapPin size={18} />
  ...
  const PrivacyView = () => (
      <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-slate-50 dark:bg-slate-900 animate-slide-in-right">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 text-center">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white">Protección de Datos</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">Tus datos están encriptados y almacenados localmente en tu dispositivo.</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
              <div className="p-4 border-b border-slate-50 dark:border-slate-700 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                      ...
                      <div>
                          <p className="font-medium text-sm text-slate-900 dark:text-white">Ubicación Precisa</p>
                          <p className="text-[10px] text-slate-400">Usada para calcular entregas</p>
                      </div>
                  </div>
              ...
              <div className="p-4 border-b border-slate-50 dark:border-slate-700 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                      ...
                      <div>
                          <p className="font-medium text-sm text-slate-900 dark:text-white">Perfil Público</p>
                          <p className="text-[10px] text-slate-400">Visible para comercios</p>
                      </div>
                  </div>
                  <div className="w-10 h-6 bg-brand-500 rounded-full relative cursor-pointer">
                      <div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full shadow-sm"></div>
              ...
              <div className="p-4 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                      ...
                      <div>
                          <p className="font-medium text-sm text-slate-900 dark:text-white">Alertas de Seguridad</p>
                          <p className="text-[10px] text-slate-400">Notificar accesos sospechosos</p>
                      </div>
                  </div>
                  <div className="w-10 h-6 bg-brand-500 rounded-full relative cursor-pointer">
                      <div className="absolute right-1 top-1 w-4 h-4 bg-white rounded-full shadow-sm"></div>
          ...
          <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl flex gap-3 items-start text-xs text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col pointer-events-none">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-[90%] max-w-sm bg-white dark:bg-slate-900 shadow-2xl pointer-events-auto animate-slide-in-right flex flex-col transition-colors duration-300">
        {/* Dynamic Header */}
        <div className="p-6 bg-slate-900 dark:bg-black text-white relative overflow-hidden shrink-0 transition-all duration-300">
             ...
             {currentView === 'MAIN' ? (
                 <div className="flex items-center gap-4 mt-2 relative z-10 animate-fade-in">
                     <div className="w-16 h-16 bg-brand-600 rounded-full border-4 border-slate-800 dark:border-slate-900 flex items-center justify-center text-2xl font-bold shadow-lg overflow-hidden">
...
const SettingItem: React.FC<{ icon: React.ReactNode; label: string; hasSwitch?: boolean; active?: boolean }> = ({ icon, label, hasSwitch, active }) => (
    <div className="flex items-center justify-between p-4 border-b border-slate-50 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700 active:bg-slate-100 dark:active:bg-slate-600 transition-colors cursor-pointer">
        ...
        {hasSwitch ? (
            <div className={`w-10 h-6 rounded-full relative transition-colors ${active ? 'bg-brand-500' : 'bg-slate-200 dark:bg-slate-600'}`}>
