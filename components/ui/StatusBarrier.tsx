...
import { Shield, Clock, Truck, Store, LogOut, MessageSquare } from 'lucide-react';
...
export const StatusBarrier: React.FC<StatusBarrierProps> = ({ type }) => {
    const { updateUser, setRole, user } = useApp();
            ...
            >
                ...
                <div className="space-y-4">
                    <h2 className="text-3xl font-black text-stone-900 dark:text-white uppercase tracking-tighter">
                        {isMerchant ? 'Comercio en Revisión' : 'Perfil en Revisión'}
                    </h2>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <Button 
                        variant="primary" 
                        fullWidth 
                        onClick={() => window.open(`https://wa.me/${user.phone || '521234567890'}`, '_blank')}
                        className="rounded-2xl h-12 text-[10px] font-black uppercase tracking-widest bg-stone-900 text-white"
                    >
                ...
                <button 
                    onClick={() => signOut()}
                    className="flex items-center gap-2 mx-auto text-[10px] font-black text-stone-400 uppercase hover:text-red-500 transition-colors"
                >
